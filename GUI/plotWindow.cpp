...
#include "GWidget.h"
#include <QClipboard>
plotWindow::plotWindow(GraphWidget *parent, QString windowTitle, QString graphTitle) :
  QMainWindow(parent),
...
QCPGraph* plotWindow::addHistogramPlot(QString name, QVector<double> t, QVector<double> y, plotformat format)
{
	...
	*/
	...
	//{
		customPlot->addGraph();
		QPen pen;
		QColor c1 = format.color;
		
		pen.setColor(QColor(c1.red(), c1.green(), c1.blue(), 200));
		customPlot->graph()->setName(name);
		customPlot->graph()->setLineStyle(format.lineStyle);
		customPlot->graph()->setPen(pen);
		customPlot->graph()->setBrush(QBrush(QColor(255 / 4.0 *customPlot->graphCount(), 160, 50, 150)));
	...
	customPlot->xAxis->setAutoTickStep(true);// false);
	...
	return customPlot->graph();
}

QCPGraph* plotWindow::addPercentilePlot(QString name, QMap<QString, double> data, plotformat format)// , plotformat format)
{
	...
	for each (QString var in vars)
		if (data.keys().contains(var))
		{
			...
			if (var == "p975") p975 = data[var];
			if (var == "p50") p50 = data[var];
			if (var == "mean") mean = data[var];
		}
	...
	// set data:
	sample1->setKey(customPlot->plottableCount());
	//sample1->setMinimum(1.1);
	sample1->setLowerQuartile(p25);
	sample1->setMedian(p50);
	sample1->setUpperQuartile(p975);
	//sample1->setMaximum(4.2);
	sample1->setOutliers(QVector<double>() << mean);
	...
	static QVector<double> tickVector;
	static QVector<QString> tickVectorLabels;
	tickVector.append(customPlot->plottableCount());
	customPlot->xAxis->setTickVector(tickVector);
	tickVectorLabels.append(name);
	customPlot->xAxis->setTickVectorLabels(tickVectorLabels);

	// prepare axes:
	//customPlot->yAxis->setLabel(QString::fromUtf8("O₂ Absorption [mg]"));
	customPlot->rescaleAxes();
	customPlot->xAxis->scaleRange(1.7, customPlot->xAxis->range().center());
	static double lscale = 1e20;
	static double uscale = -1e20;
	lscale = min(lscale, p25);
	uscale = max(uscale, p975);
	customPlot->yAxis->setRange(lscale, uscale);
	customPlot->setInteractions(QCP::iRangeDrag | QCP::iRangeZoom);
	customPlot->yAxis->setScaleType(QCPAxis::ScaleType::stLogarithmic);

	format.yAxisType = QCPAxis::ScaleType::stLogarithmic;
...
*/
	...
	pen.setStyle(format.penStyle);
	pen.setWidth(format.penWidth);
	pen.setColor(format.color);
	graph->setPen(pen);
...
void plotWindow::refreshFormat()
{
	...
	plot->yAxis->setScaleType(format[0].yAxisType);
	if (format[0].yAxisType == QCPAxis::stLogarithmic)
		plot->yAxis->setScaleLogBase(10);
	plot->legend->setVisible(format[0].legend);
	plot->xAxis->setLabel(format[0].xAxisLabel);
	plot->yAxis->setLabel(format[0].yAxisLabel);
	...
	for (int i = 0; i < format.size(); i++)
	{
		...
		plot->graph(i)->setLineStyle(format[i].lineStyle);
		plot->graph(i)->setScatterStyle(format[i].scatterStyle);
...
void plotWindow::showMenu(QMouseEvent *event)
{
...
void plotWindow::setupRealtimeDataDemo(QCustomPlot *customPlot)
{
  ...
  */
  ...
  // setup a timer that repeatedly calls plotWindow::realtimeDataSlot:
...
void plotWindow::setupItemDemo(QCustomPlot *customPlot)
{
  ...
  // setup a timer that repeatedly calls plotWindow::bracketDataSlot:
