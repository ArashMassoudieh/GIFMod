...
runtimeWindow::runtimeWindow(GraphWidget *parent, QString mode) :
  QMainWindow(parent),
...
void runtimeWindow::setLabel(QString label)
{
	ui->topLabel->setText(label);
	if (parent)
	{
		parent->log(label);
		parent->logW->writetotempfile();
	}
	...
	void runtimeWindow::setMode(QString mode)
{
	...
	else if (mode == "MCMC")
	{
		...
		ui->progressBar2->hide();
		ui->customPlot2->show();
		ui->customPlot->yAxis->setLabel("Perterbation Factor");
		ui->customPlot->yAxis->setScaleType(QCPAxis::stLogarithmic);
		ui->customPlot->yAxis->setScaleLogBase(10);
		ui->customPlot2->yAxis->setLabel("Acceptance Rate");
	}
	//setGeometry(400, 250, 542, 390);
	QCustomPlot *customPlot = ui->customPlot;
	start.start();
	setWindowTitle(QString("Simulation started at: %1").arg(QTime::currentTime().toString()));
...
//		QApplication::processEvents();
		...
		if (vars.keys().contains("mode"))
			if (vars["mode"] == "forward")
			{
				...
				{
					ui->lcdNumber->display(vars["epoch count"].toInt());
				}
			}
			else if (vars["mode"] == "inverse")
			{
				if (vars.keys().contains("progress2"))
				{
					ui->progressBar2->setValue(vars["progress2"].toInt());
				}
				if (vars.keys().contains("likelihood"))
				{
...
//				connect(ui->btnStop, SIGNAL(clicked()), this, SLOT(on_btnStop_clicked()));
				...
				if (vars.keys().contains("perterbation factor"))
				{
					...
					realtimeDataSlot(t, vars["perterbation factor"].toDouble(), false, "perterbation factor");
				}
				if (vars.keys().contains("acceptance rate"))
				{
					double t = vars["x"].toDouble();
					realtimeDataSlot(t, vars["acceptance rate"].toDouble(), true, "acceptance rate");
				}
			}
...
void runtimeWindow::on_btnStop_clicked()
{
	...
	if (QMessageBox::question(this, tr("Stop Running Model"),
...
#endif
#ifdef GWA
			parent->model->stop_triggered = true;
#endif
...
void runtimeWindow::realtimeDataSlot(double x, double y, bool secondPlot, QString chartName, bool commingBackX)
{
	...
	if (experiment)
	{
		...
		if (plot->graph(0)->data()->keys().count() > 500)
		{
			double step = (Timemax - Timemin) / 500;
			...
			{
				if (data[i].key < currentX + step / 2.0)
				{
					y += data[i].value;
					n++;
				}
	...
	if (chartName.contains("acceptance")){
...
void runtimeWindow::addScatterPlot(QString name, QVector<double> t, QVector<double> y)
{
	...
	connect(customPlot, SIGNAL(mouseRelease(QMouseEvent *)), this, SLOT(showMenu(QMouseEvent *)));
	QPen pen;
...
*/
	...
	if (start.daysTo(end) > 9) format = "MMM dd, yyyy\nhh:mm"; 
...
void runtimeWindow::addScatterPlot(QCPGraph *g)
{
	QCustomPlot *customPlot = ui->customPlot;
	QPen pen;
	QCPGraph* graph = customPlot->addGraph();
...
//	customPlot->addGraph();
	...
	pen.setWidth(1);
	pen.setColor(QColor(180, 180, 180));
	ng->setName("Confidence Band 68% ### 2");
	ng->setPen(pen);
	ng->setBrush(QBrush(QColor(255, 50, 30, 20)));
...
/*	customPlot->addGraph();
	...
	pen.setColor(Qt::red);
	customPlot->graph(1)->setPen(pen);
	customPlot->graph(1)->setName(name);
*/	/*	// add data point graph:
	customPlot->addGraph();
	customPlot->graph(3)->setPen(QPen(Qt::blue));
	customPlot->graph(3)->setLineStyle(QCPGraph::lsNone);
...
void runtimeWindow::setupQuadraticDemo(QCustomPlot *customPlot)
{
...
void runtimeWindow::setupSimpleDemo(QCustomPlot *customPlot)
{
...
void runtimeWindow::setupSincScatterDemo(QCustomPlot *customPlot)
{
...
void runtimeWindow::setupScatterStyleDemo(QCustomPlot *customPlot)
{
...
void runtimeWindow::setupLineStyleDemo(QCustomPlot *customPlot)
{
...
void runtimeWindow::setupScatterPixmapDemo(QCustomPlot *customPlot)
{
...
void runtimeWindow::setupDateDemo(QCustomPlot *customPlot)
{
  demoName = "Date Demo";
...
void runtimeWindow::setupTextureBrushDemo(QCustomPlot *customPlot)
{
...
void runtimeWindow::setupMultiAxisDemo(QCustomPlot *customPlot)
{
...
void runtimeWindow::setupLogarithmicDemo(QCustomPlot *customPlot)
{
...
void runtimeWindow::setupRealtimeDataDemo(QCustomPlot *customPlot)
{
#if QT_VERSION < QT_VERSION_CHECK(4, 7, 0)
...
//  connect(customPlot->yAxis, SIGNAL(rangeChanged(QCPRange)), customPlot->yAxis2, SLOT(setRange(QCPRange)));
  ...
  // setup a timer that repeatedly calls runtimeWindow::realtimeDataSlot:
//  connect(&dataTimer, SIGNAL(timeout()), this, SLOT(realtimeDataSlot()));
//  dataTimer.start(0); // Interval 0 means to refresh as fast as possible
}

void runtimeWindow::setupParametricCurveDemo(QCustomPlot *customPlot)
{
...
void runtimeWindow::setupBarChartDemo(QCustomPlot *customPlot)
{
...
void runtimeWindow::setupStatisticalDemo(QCustomPlot *customPlot)
{
...
void runtimeWindow::setupSimpleItemDemo(QCustomPlot *customPlot)
{
...
void runtimeWindow::setupItemDemo(QCustomPlot *customPlot)
{
#if QT_VERSION < QT_VERSION_CHECK(4, 7, 0)
  ...
  // setup a timer that repeatedly calls runtimeWindow::bracketDataSlot:
  connect(&dataTimer, SIGNAL(timeout()), this, SLOT(bracketDataSlot()));
  dataTimer.start(0); // Interval 0 means to refresh as fast as possible
}

void runtimeWindow::setupStyledDemo(QCustomPlot *customPlot)
{
...
void runtimeWindow::setupAdvancedAxesDemo(QCustomPlot *customPlot)
{
...
void runtimeWindow::setupColorMapDemo(QCustomPlot *customPlot)
{
...
void runtimeWindow::setupFinancialDemo(QCustomPlot *customPlot)
{
...
void runtimeWindow::bracketDataSlot()
{
#if QT_VERSION < QT_VERSION_CHECK(4, 7, 0)
  double secs = 0;
#else
...
void runtimeWindow::setupPlayground(QCustomPlot *customPlot)
{
  Q_UNUSED(customPlot)
}
...
void runtimeWindow::allScreenShots()
{
#if QT_VERSION < QT_VERSION_CHECK(5, 0, 0)
  QPixmap pm = QPixmap::grabWindow(qApp->desktop()->winId(), this->x()+2, this->y()+2, this->frameGeometry().width()-4, this->frameGeometry().height()-4);
#else
