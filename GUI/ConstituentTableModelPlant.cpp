...
//	node->constituentInitialCondition().append(item);
	...
	if (node->experimentName() != "All experiments")
	{
		...
		gWidget->log(QString("Limiting nutrient (%1, %2) added to block: %3 for experiment %4.").arg(item.Constituent).arg(item.Value).arg(node->Name()).arg(node->experimentName()));
		return true;
	}
	for each(QString experiment in node->parent->experimentsList())
		node->NutrientHalfSaturationConstant(experiment).append(item);
	gWidget->log(QString("Limiting nutrient half saturation constant (%1, %2) added to block: %3 for all experiments.").arg(item.Constituent).arg(item.Value).arg(node->Name()));
	return true;
}
